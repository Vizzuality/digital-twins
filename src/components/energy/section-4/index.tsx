...
export default function Section4() {
  ...
  const variantsDescription: Record<string, Variants> = {
    ...
    'step-3': {
      initial: { top: '100%', opacity: 0 },
      animate: { top: isMobile ? '10%' : '10%', opacity: 1, transition },
    }
  };
  ...
  const renderChart = <div className="relative">
    ...
    <motion.svg
      ...
      height="253"
      viewBox="0 0 492 253"
      className="absolute top-0 left-0 max-w-[100%] xl:max-h-[253px]"
      ...
      <div className="relative h-[400vh]" ref={scrollSectionRef} id="section-2-scroll-parent">
        ...
        <div className='sticky h-[100vh] w-full flex justify-center inset-0' id="section-4-1">
          ...
          {step !== 'step-1' && <motion.div
            ...
            className="absolute bottom-0 w-full text-green-950 max-w-[459px] left-8 xl:left-[60%] h-fit"
          >
            <AnimatePresence>
              {step === 'step-2' && <motion.div
                key="section-4-description-2"
                className="absolute inset-0 w-full text-2xl p-6 bg-white/30 backdrop-blur-lg h-fit"
              ...
              {step === 'step-3' && <div
                ...
                <motion.div className="absolute inset-0 w-full text-base max-w-[450px] p-6 bg-white/30 backdrop-blur-lg h-fit"
                  initial={{ opacity: 0, top: '200px' }}
                  animate={{ opacity: 1, top: 0, transition }}
                  exit={{ opacity: 0, top: '-200px' }}
                >
